...
@section('breadcrumbs')
  <div class="breadcrumbs-area clearfix">
    <h4 class="page-title pull-left">Sublease Track</h4>
    <ul class="breadcrumbs pull-left">
      <li><a href="{{ url('rcpadmin/').'/' }}">Dashboard</a></li>
      <li><span>Sublease Track</span></li>
    </ul>
  </div>
@stop
@section('content')
        ...
        <div class="card-body">
          ...
          <div class="table-responsive datatable-dark">
            <table  class="text-center table">
              ...
              <tbody>
              @if(count($subleseTrack) > 0)
                @foreach($subleseTrack as $data)
                  <tr>
                    <td> {{$data['id']}}</td>
            ...
            @if(isset($subleseTrack) && count($subleseTrack) > 0)
              {{$subleseTrack->links()}}
              Showing {{$subleseTrack->firstItem()}} to {{$subleseTrack->lastItem()}} of {{$subleseTrack->total()}}
              Entities
            @endif
          </div>
        </div>

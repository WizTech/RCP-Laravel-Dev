            ...
            <div align="center">
                <form action="{{url('rcpadmin/lead-export')}}" method="get">
                    ...
                    <select class="filter-box" name="campus_id">
                        ...
                        @if(!empty($studentViews['campuses']))
                            @foreach($studentViews['campuses'] as $campus)
                ...
                <div class="card-body">
                    <div class="table-responsive datatable-dark">
                        <table class="text-center table">
                            ...
                            <tbody>
                            @if(!empty($studentViews['leads']))
                                @foreach( $studentViews['leads'] as $lead)
                                    <tr>
                                        ...
                                        <td>{{date("Y-m-d",$lead->date_created)}}</td>
                                    </tr>
                                @endforeach
                            @endif
                            </tbody>
                        </table>
                        @if(isset($studentViews['leads']) && count($studentViews['leads']) > 0)
                            {{$studentViews['leads']->links()}}
                            Showing {{$studentViews['leads']->firstItem()}} to {{$studentViews['leads']->lastItem()}}
                            of {{$studentViews['leads']->total()}}
                            Entities
                        @endif
                    </div>
                </div>

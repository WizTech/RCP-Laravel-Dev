                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive datatable-dark">
                        <table class="text-center table">
                            ...
                            <tbody>
                            @if(!empty($career_types))
                                <?php $x = 1; ?>
                                @foreach($career_types as $careertype)
                                    <tr>
                                        <td> {{ $x }}</td>
                                        <td> {{$careertype['careers_type']}} </td>
                                        <td>
                                            <ul class="d-flex justify-content-center">
                                                <li class="mr-3">
                                                    <a href="{{ url('rcpadmin/careertype/'.$careertype['id'])}}"
                                                       class="text-secondary"><i
                                                                class="fa fa-edit"></i></a></li>
                                                <form method="POST" action="careertype/{{$careertype['id']}}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}

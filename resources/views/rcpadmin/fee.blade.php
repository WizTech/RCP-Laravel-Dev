...
@section('breadcrumbs')
  <div class="breadcrumbs-area clearfix">
    ...
    <ul class="breadcrumbs pull-left">
      ...
      <li><span>User Fee Manager</span></li>
    </ul>
  </div>
@stop
@section('content')
        ...
        <div class="card-body">
          ...
          <div class="table-responsive datatable-dark">
              ...
              @if(count($fees) > 0)
                @foreach($fees as $data)
                  <tr>
                    ...
                    <td> {{$data['reason']}} </td>
                    <td> {{date_format(date_create($data['created']), 'm/Y/d')}} </td>
                    <td> {{ ($data['type'] == 'deposit')?'User Specific - Managed from User Manager':'$'.$data['amount']}} </td>
                    <td>{{ ($data['status'] == 1) ? 'Active' : 'Inactive'}}</td>
                    <td>
                      <ul class="d-flex justify-content-center">
                        <li class="mr-3"><a
                            href="{{ url('rcpadmin/user-fee/'.$data['id'])}}"
                            class="text-secondary"><i
                              class="fa fa-edit"></i></a></li>
                        <li>
                          <form method="POST" action="user-fee/{{$data['id']}}">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
            ...
            @if(isset($fees) && count($fees) > 0)
              {{$fees->links()}}
              Showing {{$fees->firstItem()}} to {{$fees->lastItem()}}
              of {{$fees->total()}} Entities
            @endif
          </div>
        </div>
      </div>

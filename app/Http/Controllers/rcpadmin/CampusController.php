...
class CampusController extends Controller
{
        ...
        if ($q != "") {
                        ...
                        <td>
                            <ul class="d-flex justify-content-center">
                                <?php if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'property'): ?>
                                    ...
                                    <li class="mr-3"><a
                                                href="<?php echo url('rcpadmin/property/' . $campus['id'] . '/listing') ?>"
                                                class="btn btn-success btn-xs fa fa-edit" title="Campus Detail" target="_blank"></a></li>

                                <?php elseif (isset($_REQUEST['page']) && $_REQUEST['page'] == 'landing'): ?>
                                    <li class="mr-3"><a
                                                href="<?php echo url('rcpadmin/landing-page/' . $campus['id'] . '/edit') ?>"
                                                class="btn btn-success btn-xs fa fa-edit" title="Landing Page" target="_blank"></a></li>
                                <?php else: ?>

                                    <li class="mr-3"><button
                                                data-campusid="<?php echo $campus['id'] ?>" title="Edit Campus"
                                                class="btn btn-success btn-xs fa fa-pencil-square-o editCampus"> Popup </button></li>
                                    <li class="mr-3"><a href="<?php echo url('rcpadmin/campus/' . $campus['id']) ?>"
                                                        class="btn btn-success btn-xs fa fa-edit"  title="Edit Campus" target="_blank"></a></li>
                                    <li class="mr-3"><a
                                                href="<?php echo url('rcpadmin/campus/' . $campus['id'] . '/map') ?>"
                                                class="btn btn-success btn-xs fa fa-map" title="Map" target="_blank"></a></li>
                                    <li class="mr-3"><a
                                                href="<?php echo url('rcpadmin/campus/' . $campus['id'] . '/apartment') ?>"
                                                class="btn btn-primary btn-xs fa fa-building" title="Apartment" target="_blank"></a></li>
                                    <li class="mr-3"><a
            ...
            <script>
                $('.editCampus').on('click', function () {
                    ...
                    $.get("<?=env('ADMIN_URL').'/campus/edit_campus/'?>"+ campusId, function (data) {
    ...
    public function update($id, Requests\CampusRequest $request)
    {
        ...
        if (!empty($input['campus_linked'])){
            $input['campus_linked'] = implode(',',$input['campus_linked']);
        }
        ...
        if (!empty($input['campus_linked'])) {
            ...
            $campusIds = explode(',',$input['campus_linked']);
            foreach ($campusIds as $campId) {
    ...
    public function edit_campus($id)
    {
        ...
        return view('rcpadmin.campus.edit_campus', compact('campus', 'campusSelect', 'usersSelect', 'linked_campuses', 'abbrDataArray', 'zipDataArray'));
    }

    public function update_campus(Requests\CampusRequest $request, $id)
    {
        // $input = Request::all();
        $input = $request->all();
        $campus = CampusModel::find($id);
